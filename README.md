...
services:
  app:
    image: ghcr.io/verifiedjoseph/intruder-alert:1.11.0
...
| `IA_ASN_DATABASE`       | `string`  | Path of the GeoLite2 ASN database file. Ignored if `IA_MAXMIND_LICENSE_KEY` is set.     |
| `IA_COUNTRY_DATABASE`   | `string`  | Path of the GeoLite2 Country database file. Ignored if `IA_MAXMIND_LICENSE_KEY` is set. |
